...
{
    public static class SerializerBus
    {
        ...
        public static void Start()
        {
            ...
            do
            {
                ...
                _logger = log4net.LogManager.GetLogger(typeof(SerializerBus));
        ...
        private static IBusControl ConfigureBus()
        {
            var bus = MassTransit.Bus.Factory.CreateUsingRabbitMq(cfg =>
            {
                ...
                cfg.AddMessageDeserializer(CustomSerializer.CustomContentType, () => new CustomDeserializer());
                cfg.SetMessageSerializer(() => new CustomSerializer());

                cfg.ReceiveEndpoint("serializer_submit_order_queue", e =>
                {
                    /*
                     * Creates :
                 ...
                 */
                //cfg.ReceiveEndpoint("serializer_ship_order_queue", e => { e.Consumer<ShipOrderConsumer>(); });
                /*
                 * Creates :
                 ...
                 */
                //cfg.ReceiveEndpoint("serializer_bill_order_queue", e => { e.Consumer<BillOrderConsumer>(); });
            });
             ...
             * Queue : none
             */
            _sendEndpointTask = bus.GetSendEndpoint(new Uri("exchange:serializer_submit_order_queue"));
